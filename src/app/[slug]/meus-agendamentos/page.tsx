...
export default function MeusAgendamentos() {
  ...
  const handleSearch = async () => {
    ...
    try {
      const res = await fetch(
        `${process.env.NEXT_PUBLIC_API_URL}/barbershops/${slug}/appointment/check/${phone}`
      );
  ...
  const handleCancel = async (appointmentId: string) => {
    ...
    try {
      const res = await fetch(
        `${process.env.NEXT_PUBLIC_API_URL}/barbershops/${slug}/appointment/cancel/client`,
        {
          method: "PATCH",
  ...
  const handleEdit = async (app: any) => {
    ...
    const [barbersRes, servicesRes] = await Promise.all([
      fetch(`${process.env.NEXT_PUBLIC_API_URL}/barbershops/${slug}/barbers`),
  ...
  useEffect(() => {
    ...
    async function loadAvailability() {
      const res = await fetch(
        `${process.env.NEXT_PUBLIC_API_URL}/barbershops/${slug}/availability?barber_id=${editData.barber_id}&service_id=${editData.service_id}&data=${editData.appointment_date}`
      );
  ...
  const handleSaveEdit = async (appointmentId: string) => {
    try {
      const res = await fetch(
        `${process.env.NEXT_PUBLIC_API_URL}/barbershops/${slug}/appointment/${appointmentId}`,
        {
          method: "PUT",
          headers: { "Content-Type": "application/json" },
