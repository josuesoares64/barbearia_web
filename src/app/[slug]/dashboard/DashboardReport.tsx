...
export function DashboardReport({ slug, token }: { slug: string; token: string }) {
  ...
  useEffect(() => {
    async function loadBarbers() {
      ...
      try {
        const resB = await fetch(`${process.env.NEXT_PUBLIC_API_URL}/barbershops/${slug}/barbers`, { 
  ...
  const handleFinalizar = async (id: string) => {
    try {
      const res = await fetch(`${process.env.NEXT_PUBLIC_API_URL}/barbershops/${slug}/appointment/${id}/status`, {
        method: 'PATCH',
