...
function SidebarLink({
  ...
  return (
    <Link
      href={href}
      className="flex items-center gap-3 px-4 py-3 rounded-lg hover:bg-white/5 hover:text-white transition-all text-sm font-semibold"
    >
...
function MobileNavLink({
  ...
  return (
    <Link
      href={href}
      className="flex flex-col items-center gap-1 px-3 py-1 text-amber-500 hover:text-blue-600 transition-colors"
    >
      {icon}
  ...
  if (isDashboard) {
    return (
      <div className="min-h-screen flex flex-col md:flex-row bg-[#f8fafc] text-slate-900 font-sans antialiased">
        {/* SIDEBAR - DESKTOP */}
        <aside className="w-64 bg-black text-amber-500 hidden md:flex flex-col sticky top-0 h-screen shadow-2xl border-r border-white/5 shrink-0">
          ...
          <nav className="flex-1 px-4 space-y-1">
            ...
            {isOwner && (
              <>
                <SidebarLink
                  href={`/${shop?.slug}/dashboard/productForm`}
                  icon={<Package size={18} />}
                  label="Produtos"
                />
                <SidebarLink
                  href={`/${shop?.slug}/dashboard/gerencia-unidade`}
                  icon={<Scissors size={18} />}
                  label="Gerencia Unidade"
                />
                <SidebarLink
                  href={`/${shop?.slug}/dashboard/hoursManager`}
                  icon={<Clock size={18} />}
                  label="Horários de funcionamento"
                />
                <SidebarLink
                  href={`/${shop?.slug}/dashboard/pageManager`}
                  icon={<Globe size={18} />}
        ...
        <main className="flex-1 flex flex-col min-w-0 pb-20 md:pb-0">
          ...
          <header className="h-16 bg-black flex items-center px-6 md:px-8 justify-between shadow-sm sticky top-0 z-20">
            ...
            <Link href="/login">
              <button className="flex items-center gap-2 px-3 py-2 font-black text-red-500 border border-red-500/30 text-[10px] uppercase">
                Login
                <FiLogOut size={16} />
              </button>
            </Link>
