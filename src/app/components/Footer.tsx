...
const Footer = ({ shop, custom, hours }: any) => {
  ...
  const email =
    custom?.email_contato || shop?.email || "contato@barbersaas.com";
  ...
  const renderHours = () => {
    if (!hours || hours.length === 0)
      return (
        <p className="text-zinc-500 text-[10px]">Horários não informados</p>
      );
    const grouped = hours.reduce((acc: any, curr: any) => {
      if (!acc[curr.weekday]) acc[curr.weekday] = [];
      acc[curr.weekday].push(
        `${curr.open_time.slice(0, 5)} - ${curr.close_time.slice(0, 5)}`,
      );
      return acc;
    }, {});
      ...
      .map((dayIndex: any) => (
        <p key={dayIndex} className="text-zinc-400">
          <span className="font-bold text-zinc-200">
            {weekDaysMap[Number(dayIndex)]}:
          </span>{" "}
          {grouped[dayIndex].join(" / ")}
        </p>
      ));
  };
  ...
  return (
    <footer
      id="contato"
      className="w-full bg-black text-white border-t border-zinc-900"
    >
      <div className="max-w-6xl mx-auto p-8 grid grid-cols-1 gap-10 md:grid-cols-2 lg:grid-cols-4 lg:gap-8">
        ...
        <div className="flex flex-col items-start space-y-4">
          ...
          <p className="text-[10px] text-zinc-500 uppercase font-bold tracking-widest leading-tight">
            A melhor experiência de <br /> cuidado masculino. <br />© 2026 —
            GESTÃO PROFISSIONAL.
          </p>
        </div>
        ...
        <div className="space-y-3">
          <h3 className="text-sm font-black uppercase text-amber-500 mb-4">
            Contato
          </h3>
          <a
          ...
          {custom?.instagram_url && (
            <a
              className="flex items-center space-x-2 text-zinc-400 hover:text-white transition text-xs"
              // Ajuste aqui: usa a variável correta e garante o link externo
              href={
                custom.instagram_url.startsWith("http")
                  ? custom.instagram_url
                  : `https://instagram.com/${custom.instagram_url.replace("@", "")}`
              }
              target="_blank"
              rel="noopener noreferrer"
            >
        ...
        <div className="space-y-3">
          ...
          <div className="text-[11px] space-y-1">{renderHours()}</div>
        </div>
      </div>

      <div className="w-full border-t border-zinc-900 py-6 text-center">
        <a
          href="https://josuesoaresdev.vercel.app/"
          target="_blank"
          rel="noopener noreferrer"
        >
